...
After the backend update is deployed and the workflow/helper are on the default branch, use **Actions → RSS Ingestion → Run workflow**. Confirm numeric completion counts. A validated completed pipeline with third-party feed failures succeeds with warnings naming the affected sources. Configuration, transport, invalid completion, and pipeline/database failures still fail the job. All sources being not due is a valid zero-work completion. Inspect source health rather than repeatedly rerunning partial failures.
